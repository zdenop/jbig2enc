...
def parse_palette(format, mode, spal):
    per_color = len(mode)
    num_colors = int(len(spal) / per_color)
    ret = []
    really_rgb = False
    ...
    for i in range(0, num_colors):
        ...
        if mode == "CMYK":
            ...
            if format == "tiff":
                color = (
                    ord(spal[i]),
                    ord(spal[i + num_colors]),
                    ord(spal[i + num_colors * 2]),
                )
            else:
            ...
            if not really_rgb and (
                color[0] != color[1] or color[1] != color[2] or color[0] != color[2]
            ):
                really_rgb = True
...
def main(pagefiles):
    ...
    creator = Obj({"Subtype": "/Artwork", "Creator": "(jbig2)", "Feature": "(Layers)"})
    ...
    for p in pagefiles:
        ...
        if bg_type != "":
            ...
            contents.reinit(
                {"Filter": "/FlateDecode"},
                zlib.compress(
                    "q %d 0 0 %d 0 0 cm /Im2 Do Q q %d 0 0 %d 0 0 cm /Im1 Do Q"
            ...
            if cspace == "/Indexed":
                (mode, cpal) = parse_palette(
                    bg_type, im.palette.mode, im.palette.tostring()
                )
